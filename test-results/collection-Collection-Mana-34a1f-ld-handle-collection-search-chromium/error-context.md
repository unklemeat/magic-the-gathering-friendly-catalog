# Page snapshot

```yaml
- generic [ref=e1]:
  - navigation [ref=e2]:
    - generic [ref=e3]:
      - heading "MTG Friendly Catalog" [level=1] [ref=e4] [cursor=pointer]
      - generic [ref=e5]:
        - combobox [ref=e6]:
          - option "Italiano" [selected]
          - option "English"
        - button "API Ok" [ref=e9] [cursor=pointer]
  - main [ref=e10]:
    - generic [ref=e12]:
      - generic [ref=e13]:
        - button "Cerca Carta" [active] [ref=e14] [cursor=pointer]
        - button "Mazzi" [ref=e15] [cursor=pointer]
        - button "Esplora Espansioni" [ref=e16] [cursor=pointer]
        - button "Carica CSV" [ref=e17] [cursor=pointer]
        - button "Gestisci Dati" [ref=e18] [cursor=pointer]
      - generic [ref=e20]:
        - heading "I Miei Mazzi" [level=2] [ref=e21]
        - generic [ref=e22]:
          - textbox "Inserisci il nome del nuovo mazzo..." [ref=e23]
          - button "Crea Mazzo" [ref=e24] [cursor=pointer]
  - generic [ref=e26]:
    - paragraph [ref=e27]: Inserisci un nome di carta per la ricerca.
    - button "modalOk" [ref=e29] [cursor=pointer]
```